...
struct CountThenVec<T: DekuRead<deku::ctx::Endian>> {
    #[allow(dead_code)]
    count: u32,
...
impl<T: DekuRead<deku::ctx::Endian>> CountThenVec<T> {
    pub fn map(self) -> Result<Vec<T>, DekuError> {
        Ok(self.vec)
    }
}
...
impl Metadata {
    fn read_vec(mut rest: &BitSlice<Msb0, u8>) -> Result<(&BitSlice<Msb0, u8>, Vec<Metadata>), DekuError> {
        #[derive(DekuRead)]
        #[deku(endian = "big")]
        ...
        loop {
            ...
            let (newer_rest, metadata) = Metadata::read(new_rest, (header.block_type, header.size))?;
